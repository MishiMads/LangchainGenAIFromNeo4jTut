...
class OpenAIJudge(DeepEvalBaseLLM):
    def __init__(self, model="gpt-4o-mini"):
        self.model_name = model
        self.chat = ChatOpenAI(model=model, temperature=0, model_kwargs={"response_format": {"type": "json_object"}})

    def load_model(self):
        return self.chat

    def _parse_json_response(self, content: str, schema):
        ...
        try:
            ...
            print(f"Parsed data: {data if 'data' in locals() else 'N/A'}")
            return None

    def generate(self, prompt: str, schema=None) -> str:
        res = self.chat.invoke(prompt).content
        if schema:
            return self._parse_json_response(res, schema)
        return res
...
# Global list to store results for Excel export
test_results = []
...
def test_math_rag(data):
    ...
    # Metrics will be evaluated by assert_test()
    print("\n⚡ Preparing to evaluate metrics...")
    print()  # Extra newline for readability

    # Collect metric scores for Excel export
    result_entry = {
        "Category": data.get("category", "General"),
        "Question": query,
        "Expected_Output": expected,
        "Actual_Output": actual_output,
        "Context": context_str[:200] + "..." if len(context_str) > 200 else context_str,
    }
    ...
    # First, measure all metrics
    print("\n⚡ Evaluating metrics...")
    for metric in metrics:
        try:
            metric.measure(test_case)
            print(f"✅ Evaluated: {getattr(metric, 'name', metric.__class__.__name__)}")
        except Exception as e:
            print(f"⚠️ Error evaluating {getattr(metric, 'name', metric.__class__.__name__)}: {e}")

    # Now print detailed results
    print("\n" + "=" * 60)
    ...
    for metric in metrics:
        ...
        # Check if metric was successfully evaluated
        if hasattr(metric, 'score') and metric.score is not None:
            ...
            # Save to Excel
            result_entry[f"{metric_name}_Score"] = round(metric.score, 3)
            result_entry[f"{metric_name}_Threshold"] = metric.threshold
            result_entry[f"{metric_name}_Pass"] = "✅" if metric.score >= metric.threshold else "❌"
            if hasattr(metric, 'reason') and metric.reason:
                result_entry[f"{metric_name}_Reason"] = metric.reason
        else:
            # Metric failed to evaluate
            print(f"\n❌ METRIC ERROR: {metric_name}")
            if hasattr(metric, 'error') and metric.error:
                print(f"Error: {metric.error}")
            print("-" * 60)

            result_entry[f"{metric_name}_Score"] = "N/A"
            result_entry[f"{metric_name}_Threshold"] = metric.threshold
            result_entry[f"{metric_name}_Pass"] = "❌ ERROR"
            if hasattr(metric, 'error') and metric.error:
                result_entry[f"{metric_name}_Reason"] = str(metric.error)
...
# Tror dog ikke det virker så godt med confident AI så
    ...
    try:
        ...
        result_entry["Overall_Status"] = "✅ PASSED" if test_passed else "❌ FAILED"
        result_entry["Failure_Message"] = failure_message
        test_results.append(result_entry)


# ==========================================
# 5. PYTEST HOOKS - SAVE RESULTS TO EXCEL
# ==========================================

def pytest_sessionfinish(session, exitstatus):
    """
    Called after whole test run finished, right before returning exit status.
    This is the perfect place to save our test results to Excel.
    """
    if test_results:
        ...
        first_cols = ["Overall_Status", "Category", "Question", "Expected_Output", "Actual_Output"]
        ...
        try:
            ...
            print(f"Passed: {sum(1 for r in test_results if r['Overall_Status'] == '✅ PASSED')}")
            print(f"Failed: {sum(1 for r in test_results if r['Overall_Status'] == '❌ FAILED')}")
            print(f"{'='*60}\n")

        except Exception as e:
            print(f"\n⚠️ WARNING: Failed to save Excel file: {e}")
            # Fallback to CSV if Excel fails
            csv_filename = f"test_results_{timestamp}.csv"
            df.to_csv(csv_filename, index=False, encoding='utf-8-sig')
            print(f"✅ Results saved as CSV instead: {csv_filename}\n")
    else:
